...
from .cnn import CNN
from myweb0415.encoder.module.pool import MaxPool
    ...
    def tokenize(self, item):
        ...
        if not is_token:
            if pos_head[0] > pos_tail[0]:
                ...
                rev = False
            sent_0 = self.tokenizer.tokenize(sentence[:pos_min[0]])#0句子
            sent_1 = self.tokenizer.tokenize(sentence[pos_min[1]:pos_max[0]])#1句子
            sent_2 = self.tokenizer.tokenize(sentence[pos_max[1]:])#2句子
            ent_0 = self.tokenizer.tokenize(sentence[pos_min[0]:pos_min[1]])#1实体
            ent_1 = self.tokenizer.tokenize(sentence[pos_max[0]:pos_max[1]])#2实体
            tokens = sent_0 + ent_0 + sent_1 + ent_1 + sent_2
            if rev:
                pos_tail = [len(sent_0), len(sent_0) + len(ent_0)]#各自的位置
