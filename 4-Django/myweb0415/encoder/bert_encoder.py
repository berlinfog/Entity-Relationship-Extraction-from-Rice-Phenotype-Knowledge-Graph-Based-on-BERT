    ...
    def tokenize(self, item):
        ...
        if not is_token:
            ...
            if pos_head[0] > pos_tail[0]:
                ...
                rev = False
            sentence0 = self.tokenizer.tokenize(sentence[:pos_min[0]])
            at0 = self.tokenizer.tokenize(sentence[pos_min[0]:pos_min[1]])
            sentence1 = self.tokenizer.tokenize(sentence[pos_min[1]:pos_max[0]])
            at1 = self.tokenizer.tokenize(sentence[pos_max[0]:pos_max[1]])
            sentence2 = self.tokenizer.tokenize(sentence[pos_max[1]:])
            if self.mask_entity:
                at0 = ['[unused4]']
                at1 = ['[unused5]']
                if rev:
                    at0 = ['[unused5]']
                    at1 = ['[unused4]']
            pos_head = [len(sentence0), len(sentence0) + len(at0)]
            pos_tail = [
                len(sentence0) + len(at0) + len(sentence1),
                len(sentence0) + len(at0) + len(sentence1) + len(at1)
            ]
            if rev:
                pos_tail = [len(sentence0), len(sentence0) + len(at0)]
                pos_head = [
                    len(sentence0) + len(at0) + len(sentence1),
                    len(sentence0) + len(at0) + len(sentence1) + len(at1)
                ]
            tokens = sentence0 + at0 + sentence1 + at1 + sentence2
        else:
            tokens = sentence
